    ...
    var Day_SVGMove = function (trans) {
        ...
        for (var i = 0; i < DayShowCount; i++) {
            ...
            for (var j = 0; j < TimeDateCount; j++) {
                ...
                var SVG_Day = '   <g class="tick_Year_One" transform="translate(' + Day_Trans_min + ')" >' + '<rect class="DayRect Btn_DayRect" x="0.2" y="0" width="12.5" height="55" value="' + ShowYear + '-' + (parseInt(ShowMonth) + 1) + '-' + parseInt(j + 1) + '">' + '<title>' + ShowYear + '-' + (parseInt(ShowMonth) + 1) + '-' + parseInt(j + 1) + '</title>' + '</rect>' + '<line class="tick_dot" x1="12" x2="12" y1="0" y2="52"></line>' + '</g>';
                ...
                if (DataInfoList) {
                    DataInfoList.forEach(function (DataTimeItem) {
                        if (isFindExist === false) {
                            ...
                            if (TimeSelect.isBetween(BeginTime, EndTime) || TimeSelect.isSame(BeginTime) || TimeSelect.isSame(EndTime)) {
                                isFindExist = true;
