...
Title.prototype.Create = function (doc) {
   ...
   this.grp = doc.createElement("g"), this.grp.setAttribute("transform", "translate(0,0)");
...
Title.Register = function Title_Register(elem) {
   if (TitleElementOf(elem) != null) elem.addEventListener("mouseover", Title.Activate, false);
};
...
function TitleElementOf(elem) {
   ...
   for (var i = 0; i < childs.getLength(); i++) if (childs.item(i).getNodeType() == 1 && childs.item(i).getNodeName() == "title") // title element ..
...
function TextOf(elem) {
   ...
   for (var i = 0; childs && i < childs.getLength(); i++) if (childs.item(i).getNodeType() == 3) // text node ..
...
function AddTitleEvents(elem) {
   ...
   for (var i = 0; i < childs.getLength(); i++) if (childs.item(i).getNodeType() == 1) // element node ..
