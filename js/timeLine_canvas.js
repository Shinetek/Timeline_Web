...
var timeLine = {
    ...
    initialize: function (elem, options) {
        ...
        this.options = {
            ...
            mode_list: ['year', 'month', 'day', 'hour', 'minute'],
            //在当前模式下，选择精确到的 单位
            mode_unit: ['month', 'day', 'hour', 'minute', 'second'],
            //每一个单位大小
            mode_timespan: [12.5, 12.5, 12.5, 12.5, 5, 2.5],
    ...
    _getTimeSpace: function (trans) {
        ...
        //每一个单位大小
        var selectmodetimespan = this.options.mode_timespan  [selectMode];
        //    console.log(timeLine.options.moment_begin.format("YYYY-MM-DD HH:mm:ss"));
    ...
    _drag: function (trans) {
        ...
        timeLine._draw_timeline(moment_begin, moment_end, this.options.select_date, "day", 12.5);
    ...
    _draw_timeline: function (begin_date, end_date, select_date, unit_str, unit_pix) {
        ...
        //绘制横线
        timeLine._canvas_line("white", 0, 55, timeLine.options.timeline_width, 55);
        //绘制年间隔
        ...
        while (moment_draw < end_date) {
            ...
            if (timeLine._getArcMode(moment_draw)) {
                timeLine._canvas_arc("white", trans - unit_pix, 55, 7);
                timeLine._canvas_line("white", trans - unit_pix, 3, trans - unit_pix, 75);
                timeLine._canvas_txt("white", moment_draw.format("YYYY-MM-DD HH:mm:ss"), trans - unit_pix + 2, 75);
            }

            if (timeLine._getLongLineMode(moment_draw)) {
                timeLine._canvas_line("white", trans, 35, trans, 55);
            }
            else {
                // (fillcolor, begin_point_x, begin_point_y, end_point_x, end_point_y)
                timeLine._canvas_line("white", trans, 45, trans, 55);
            }
